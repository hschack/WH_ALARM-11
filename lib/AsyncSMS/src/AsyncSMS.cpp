...
void AsyncSMS::reinitGSMModuleConnection() {
	...
	enqueue("AT+CREG=1");
	enqueue("AT+CLTS=1"); //HS Set rtc to be updatet from GSM
	enqueue("AT&W"); //HS Save in SIM800L
	enqueue("ATE0");
...
void AsyncSMS::process() {
	if (!_receiving && !_waitingForResponse) {
		if (queueAvailable()) {
			...
			#ifdef ASYNC_SMS_DEBUG_MODE
			Serial2.println("deque occurred");
			Serial2.println(cmd);
			Serial2.println(_cmdQueueStart);
			#endif
			if (cmd == "SMS") {
	...
	if (_receivingMessageTimer.available()) {
		...
		#ifdef ASYNC_SMS_DEBUG_MODE
		Serial2.println(_receivedMessageIndex);
		for (uint16_t i = 0 ; i < _receivedMessageIndex ; i ++){
			Serial2.print(_receivedMessage[i]);
			Serial2.print(" ");
		}
		Serial2.println("");
		#endif
...
void AsyncSMS::handleCommandResponse() {
	if (isOK())
	  ...
	  else if (checkFunctionResult("+CCLK:")) {  // By hs for read GSM Time
		log("CCLK");
		String res = getResultParameter("+CCLK:");
		parseResultValues(res);
	    res.toCharArray(timeBuffer, 22);
		//_state[4] = _resValues[0];
		//_state[5] = _resValues[1];
	} 
	  else 
		log("NIEZNNAE");
}
...
void AsyncSMS::deleteReadSMS() {
	enqueue("AT+CMGDA=DEL READ");
}

void AsyncSMS::deleteSendSMS() {
	enqueue("AT+CMGDA=DEL SEND");
}

void AsyncSMS::readGSMTime() {
	enqueue("AT+CCLK?");
}
